import { agentViz_METRIC_COUNTERS, IMetricsProvider } from '../Interface.Metrics'
...
import { getVersion } from 'agentViz-components'
...
export class OpenTelemetry implements IMetricsProvider {
    ...
    async initializeCounters(): Promise<void> {
        try {
            ...
            const agentVizVersion = await getVersion()

            this.resource = new Resource({
                [ATTR_SERVICE_NAME]: process.env.METRICS_SERVICE_NAME || 'agentVizAI',
                [ATTR_SERVICE_VERSION]: agentVizVersion.version // Version as a label
            })
            ...
            const meter = this.meterProvider.getMeter('agentViz-metrics')
            // look at the agentViz_COUNTER enum in Interface.Metrics.ts and get all values
            // for each counter in the enum, create a new promClient.Counter and add it to the registry
            const enumEntries = Object.entries(agentViz_METRIC_COUNTERS)
            ...
            try {
                ...
                if (!createdMetrics.has('agentViz_version')) {
                    const versionGuage = meter.createGauge('agentViz_version', {
                        description: 'agentViz version'
                    })
                    // remove the last dot from the version string, e.g. 2.1.3 -> 2.13 (gauge needs a number - float)
                    const formattedVersion = agentVizVersion.version.replace(/\.(\d+)$/, '$1')
                    versionGuage.record(parseFloat(formattedVersion))
                    createdMetrics.set('agentViz_version', true)
                }
            } catch (error) {
