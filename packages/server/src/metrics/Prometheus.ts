...
export class Prometheus implements IMetricsProvider {
    ...
    async initializeCounters(): Promise<void> {
        const serviceName: string = process.env.METRICS_SERVICE_NAME || 'agentVizAI'
        ...
        try {
            const versionGaugeCounter = new promClient.Gauge({
                name: 'agentViz_version_info',
                help: 'agentViz version info.',
                labelNames: ['version'],
                registers: [this.register] // Explicitly set the registry
            })
            ...
            this.counters.set('agentViz_version', versionGaugeCounter)
        } catch (error) {
            // If metric already exists, get it from the registry
            const existingMetric = this.register.getSingleMetric('agentViz_version')
            if (existingMetric) {
                this.counters.set('agentViz_version', existingMetric as promClient.Gauge<string>)
            }
        }
    ...
    public incrementCounter(counter: agentViz_METRIC_COUNTERS, payload: any) {
    ...
    private registerMetrics() {
        if (process.env.METRICS_INCLUDE_NODE_METRICS !== 'false') {
            ...
            promClient.collectDefaultMetrics({
                ...
                prefix: 'agentViz_' // Add a prefix to avoid conflicts
            })
        }
