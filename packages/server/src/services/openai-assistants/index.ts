...
import { getFileFromUpload, removeSpecificFileFromUpload } from 'agentViz-components'
...
const getAllOpenaiAssistants = async (credentialId: string): Promise<any> => {
    try {
        ...
        if (!credential) {
            throw new InternalagentVizError(StatusCodes.NOT_FOUND, `Credential ${credentialId} not found in the database!`)
        }
