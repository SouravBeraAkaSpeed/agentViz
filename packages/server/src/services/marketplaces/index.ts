...
const getAllTemplates = async () => {
    try {
        ...
        const agentVizDocsQnAIndex = sortedTemplates.findIndex((tmp) => tmp.templateName === 'agentViz Docs QnA')
        if (agentVizDocsQnAIndex > 0) {
            sortedTemplates.unshift(sortedTemplates.splice(agentVizDocsQnAIndex, 1)[0])
        }
