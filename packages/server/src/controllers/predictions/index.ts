...
const createPrediction = async (req: Request, res: Response, next: NextFunction) => {
    try {
        ...
        if (!chatflow) {
            throw new InternalagentVizError(StatusCodes.NOT_FOUND, `Chatflow ${req.params.id} not found`)
        }
        ...
        if (isDomainAllowed) {
            ...
            throw new InternalagentVizError(StatusCodes.FORBIDDEN, unauthorizedOriginError)
        }
    } catch (error) {
        next(error)
    }
