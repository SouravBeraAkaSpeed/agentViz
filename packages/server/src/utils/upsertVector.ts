...
export const executeUpsert = async ({
    ...
    if (vsNodesWithFileUpload.length > 1) {
        throw new InternalagentVizError(StatusCodes.INTERNAL_SERVER_ERROR, 'Multiple vector store nodes with fileUpload enabled')
    ...
    if (vsNodes.length > 1 && !stopNodeId) {
        ...
        throw new InternalagentVizError(StatusCodes.NOT_FOUND, 'No vector node found')
    }
...
export const upsertVector = async (req: Request, isInternal: boolean = false) => {
    ...
    try {
        ...
        if (process.env.MODE === MODE.QUEUE) {
            ...
            appServer.metricsProvider?.incrementCounter(agentViz_METRIC_COUNTERS.VECTORSTORE_UPSERT, {
        ...
        appServer.metricsProvider?.incrementCounter(agentViz_METRIC_COUNTERS.VECTORSTORE_UPSERT, { status: agentViz_COUNTER_STATUS.FAILURE })
