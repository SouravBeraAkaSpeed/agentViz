...
export abstract class BaseCommand extends Command {
    static flags = {
        agentViz_USERNAME: Flags.string(),
        agentViz_PASSWORD: Flags.string(),
        agentViz_FILE_SIZE_LIMIT: Flags.string(),
        PORT: Flags.string(),
        ...
        agentViz_SECRETKEY_OVERWRITE: Flags.string(),
    ...
    protected onTerminate() {
        return async () => {
            try {
                ...
                setTimeout(async () => {
                    logger.info('agentViz was forced to shut down after 30 secs')
                    await this.failExit()
                }, 30000)
    ...
    async init(): Promise<void> {
        ...
        // Authorization
        if (flags.agentViz_USERNAME) process.env.agentViz_USERNAME = flags.agentViz_USERNAME
        if (flags.agentViz_PASSWORD) process.env.agentViz_PASSWORD = flags.agentViz_PASSWORD
        ...
        // API Configuration
        if (flags.agentViz_FILE_SIZE_LIMIT) process.env.agentViz_FILE_SIZE_LIMIT = flags.agentViz_FILE_SIZE_LIMIT
        ...
        if (flags.agentViz_SECRETKEY_OVERWRITE) process.env.agentViz_SECRETKEY_OVERWRITE = flags.agentViz_SECRETKEY_OVERWRITE
