...
export default class Worker extends BaseCommand {
    ...
    async run(): Promise<void> {
        logger.info('Starting agentViz Worker...')
    ...
    async stopProcess() {
        try {
            ...
            logger.info(`Shutting down agentViz Prediction Worker ${this.predictionWorkerId}...`)
            await predictionWorker.close()

            const upsertWorker = queueManager.getQueue('upsert').getWorker()
            logger.info(`Shutting down agentViz Upsertion Worker ${this.upsertionWorkerId}...`)
            await upsertWorker.close()
        } catch (error) {
            logger.error('There was an error shutting down agentViz Worker...', error)
            await this.failExit()
        }
