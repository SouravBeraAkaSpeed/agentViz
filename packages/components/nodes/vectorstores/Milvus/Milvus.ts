...
class Milvus_VectorStores implements INode {
    ...
    async init(nodeData: INodeData, _: string, options: ICommonObject): Promise<any> {
        ...
        if (isFileUploadEnabled && options.chatId) {
            if (milvusFilter) milvusFilter += ` OR ${agentViz_CHATID} == "${options.chatId}" OR NOT EXISTS(${agentViz_CHATID})`
            else milvusFilter = `${agentViz_CHATID} == "${options.chatId}" OR NOT EXISTS(${agentViz_CHATID})`
        }
